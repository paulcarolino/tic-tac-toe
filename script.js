const gameBoard = (() => {
  ...
  const resetGame = () => {
    ...
    document.getElementById("title-announcement").innerHTML =
      "<h1>TIC TAC TOE</h1>";
  ...
  const checkWinner = () => {
    ...
    if (gameOver) {
      ...
      if (document.getElementsByClassName(`winner-${currPlayer.sign}`).length) {
        document.getElementById("title-announcement").innerText =
          currPlayer.name.toString().toUpperCase() + " WINS!";
      } else {
...
document
  ...
  .addEventListener("click", gameBoard.setGame);
